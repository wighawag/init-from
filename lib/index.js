...
gittar
	...
	.then(file => {
		...
		gittar.extract(file, dest).then(() => {
			const variables = { name: 'test', test: true };
			...
			function traverse(dir, result = [], topDir, step = 0) {
				...
				dirFiles.forEach(name => {
					...
					const newName = template(variables);
					console.log(`${name} -> ${newName}`);
					if (newName == '') {
						console.log(step, `deleting ${fileStats.name} ...`);
						rimraf.sync(fileStats.path);
					} else {
						const newPath = path.join(path.dirname(fileStats.path), newName);
						console.log(
							step,
						...
						if (fileStats.directory) {
							...
							return traverse(fileStats.path, result, topDir || dir, ++step);
						}
						result.push(fileStats);
					}
				});
			...
			for (const file of files) {
				if (!file.directory) {
					...
					const newContent = template(variables);
