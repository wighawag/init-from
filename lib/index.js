...
if (args.length < 2) {
	throw new Error(
		'need to specify both a repo and a destintion folder : `$ init-from <repo> <target> [options]`'
	);
}
...
for (const arg of args) {
	if (arg.startsWith('-')) {
		if (counter == 0 || counter == 1) {
			throw new Error(
				'options needs to be at the end: `$ init-from <repo> <target> [options]`'
			);
		}
		if (composed !== '') {
...
if (flags.help || !repo) {
  ...
  ${_('Examples')}
    $ init-from vutran/boiler boiler-test
    $ init-from vutran/boiler my-app
    $ init-from vutran/boiler#v1.2.0 myapp
    $ init-from gitlab:vutran/boiler myapp
    $ init-from vutran/boiler myapp --cache
	`);
	...
	.then(file => {
		...
		gittar
			...
			.then(() => {
				...
				let configContent;
				try {
					configContent = fs.readFileSync(dest + '/.init-from.json').toString();
				} catch (e) {
					try {
						configContent = fs.readFileSync(dest + '/.zel').toString();
					} catch (e) {}
				}
				let config = {};
				if (configContent) {
				...
				if (typeof variables.name === 'undefined') {
					variables.name = changeCase.capitalCase(flags._[1]);
				}
				...
				for (const file of files) {
					if (!file.directory && !isBinary(file.path)) {
